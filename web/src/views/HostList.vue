<template>
  <div class="h-full flex flex-col">
    ...
    <div v-else class="bg-white rounded-lg shadow overflow-hidden flex-1 flex flex-col min-h-0">
          ...
          <tbody class="bg-white divide-y divide-gray-200">
            <tr v-for="host in hosts" :key="host.id" class="hover:bg-gray-50">
                ...
                <span
                  :class="{
                    ...
                    'bg-gray-100 text-gray-800':
                      host.health_status === 'unknown' || !host.health_status,
                  }"
                >
                  {{
              ...
              <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                ...
                <button @click="openEditModal(host)" class="text-blue-600 hover:text-blue-900 mr-3">
                  编辑
                </button>
                <button
                  @click="openCopyModal(host)"
                  class="text-violet-600 hover:text-violet-900 mr-3"
                >
                  复制
                </button>
                <button @click="confirmDelete(host)" class="text-red-600 hover:text-red-900">
                  删除
                </button>
              </td>
            </tr>
      ...
      >
        <div class="text-sm text-gray-500">共 {{ total }} 条记录，第 {{ page }} 页</div>
        ...
        <form @submit.prevent="saveHost" class="p-6" novalidate>
          ...
          <div class="mb-4">
            ...
            <input
              ...
              placeholder="输入用户名"
            />
            <p v-if="fieldErrors.username" class="text-xs text-red-600 mt-1">
              {{ fieldErrors.username }}
            </p>
          </div>
          ...
          <div class="mb-4">
            <label class="block text-sm font-medium text-gray-700 mb-1"
              >Auth Method <span class="text-red-500">*</span></label
            >
            <select
            ...
            <p v-if="isCopying" class="text-xs text-gray-500 mt-1">
              与源主机一致；密文在服务端复制，不经浏览器
            </p>
            <p v-else-if="isEditing" class="text-xs text-amber-600 mt-1">
              <span v-if="form.auth_method !== originalAuthMethod"
                >⚠️ 认证方式已变更，必须填写新的密码/私钥</span
              >
          ...
          <div class="mb-4">
            ...
            <p v-else-if="isEditing && authDataRequired" class="text-xs text-amber-600 mb-2">
              ⚠️ 认证方式已从「{{ originalAuthMethod === 'password' ? '密码' : '私钥' }}」变更为「{{
                form.auth_method === 'password' ? '密码' : '私钥'
              }}」，必须填写新的{{ form.auth_method === 'password' ? '密码' : '私钥' }}
            </p>
            <input
            ...
            <textarea
              ...
              "
            ></textarea>
            <p v-if="fieldErrors.auth_data" class="text-xs text-red-600 mt-1">
              {{ fieldErrors.auth_data }}
            </p>
          </div>
          ...
          <div class="mb-6">
            ...
            />
            <p v-if="fieldErrors.weight" class="text-xs text-red-600 mt-1">
              {{ fieldErrors.weight }}
            </p>
    ...
    >
      <div class="bg-white rounded-lg shadow-xl w-full max-w-sm mx-4 p-6">
        ...
        <p class="text-gray-600 mb-6">
          确定要删除主机
          <span class="font-semibold">{{ deleteTarget?.name }}</span> 吗？此操作不可恢复。
        </p>
        <div class="flex justify-end space-x-3">
          <button
...
const validateHostForm = (): boolean => {
  ...
  if (authDataRequired.value && !String(form.value.auth_data || '').trim()) {
    if (isEditing.value && form.value.auth_method !== originalAuthMethod.value) {
      ...
      setFieldError(
        'auth_data',
        `认证方式已从「${fromMethod}」变更为「${toMethod}」，请填写新的${toMethod}`
      )
    } else {
