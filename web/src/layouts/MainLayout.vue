<template>
  <div class="flex h-viewport min-h-0 bg-gray-100 overflow-hidden" style="overflow-x: hidden;">
        ...
        <ul class="space-y-1 px-3">
          ...
          <li>
            ...
            >
              <svg
                class="w-5 h-5"
                :class="{ 'mr-3': !isMobile || sidebarCollapsed }"
                fill="none"
                stroke="currentColor"
                viewBox="0 0 24 24"
              >
    ...
    <div class="flex-1 flex flex-col min-w-0 overflow-hidden">
      <!-- 顶部栏 -->
      <header
        class="h-16 bg-white border-b border-gray-200 flex items-center justify-between px-4 lg:px-6 flex-shrink-0"
      >
        ...
        <div class="flex items-center space-x-4">
          ...
          <div class="flex items-center gap-2 px-3 py-1.5 bg-gray-100 rounded-full">
            <div
              :class="['w-2.5 h-2.5 rounded-full', wsConnected ? 'bg-green-500' : 'bg-red-500']"
            ></div>
          ...
          <button class="text-sm text-red-600 hover:text-red-800 font-medium" @click="handleLogout">
            登出
          </button>
        </div>
      </header>
